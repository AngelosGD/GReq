    ...
    <aside className="w-60 flex-shrink-0 border-r border-zinc-200/50 flex flex-col">
        ...
        {(['Todas', ...methods] as const).map((m) => {
          ...
          return (
            <button
              ...
              className={`px-2 py-1 rounded-md text-[10px] font-medium flex items-center gap-1.5 transition-all ${act ? 'text-zinc-800' : 'text-zinc-400 hover:text-zinc-600'}`}
            >
              {c ? (
      ...
      <div className="px-3 pt-2 pb-1">
        <div className="relative">
          ...
          <input value={search} onChange={(e) => onSetSearch(e.target.value)} placeholder="Buscar..." className="w-full bg-zinc-50 border border-zinc-200/50 rounded-lg pl-7 pr-2.5 py-1.5 text-[11px] text-zinc-600 placeholder-zinc-300 outline-none focus:border-zinc-300 transition-all" />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1 space-y-0.5">
        {mockApis.length === 0 ? (
          ...
          <div className="text-center py-8"><p className="text-[11px] text-zinc-400">Sin resultados</p></div>
        ) : (
          [...filtered].sort((a, b) => (a.pinned === b.pinned ? 0 : a.pinned ? -1 : 1)).map((api) => {
            const isSelected = activeTabApiId === api.id
            return (
              <div key={api.id} className={`w-full px-1.5 py-1.5 rounded-lg transition-all flex items-center gap-1 ${isSelected ? 'bg-zinc-100' : 'hover:bg-zinc-50'}`}>
      ...
      <div className="px-2 pb-2 border-t border-zinc-100 pt-2">
        <button
          onClick={onNewApi}
          className="flex items-center justify-center gap-1.5 w-full px-3 py-2 rounded-lg text-[11px] font-medium bg-zinc-900 text-white hover:bg-zinc-800 active:scale-[0.98] transition-all"
        >
