    ...
    <aside className="w-60 flex-shrink-0 border-r border-zinc-200/60 dark:border-zinc-700/60 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1 space-y-0.5">
        {mockApis.length === 0 ? (
          ...
          const grouped = new Map<string, MockApiItem[]>()
          for (const api of sorted) {
            const col = api.collection || ''
            if (!grouped.has(col)) grouped.set(col, [])
            grouped.get(col)!.push(api)
          }
          const entries = [...grouped.entries()].sort(([a], [b]) => a === '' ? 1 : b === '' ? -1 : a.localeCompare(b))
          return entries.flatMap(([col, apis], gi) => [
            col ? (
              <div key={`h-${gi}`} className="px-1.5 pt-3 pb-1 text-[9px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">
                {col}
              </div>
            ) : null,
            ...apis.map((api) => {
